...
- **Search** the skills.sh and agentskill.sh marketplaces, or your own installed skills
...
<iframe width="100%" style={{"aspect-ratio": "12 / 9"}}
        src="https://www.youtube.com/embed/Rhe7YZ6zRWg?si=YQtJrOONEhtPgcFw"
        title="ai-skills demo" frameborder="0"
...
Most ai-skills commands support **interactive mode** - just run the command without any flags and you'll be guided through each step with prompts. This is the recommended way to use ai-skills, as you don't need to memorise any flags or parameters. In any multi-select prompt, press **Shift+Tab** to toggle select/deselect all. Multi-select prompts require at least one selection - pressing Enter with nothing selected shows `Please select at least one, or press Ctrl+C to cancel.` instead of cancelling the operation.

```bash
aiskills install owner/repo   # Prompts for agent(s) and location
aiskills search               # Search local skills or the marketplace
...
e.g.) Install skills from Anthropic's skills repo:

```bash
aiskills install anthropics/skills
```

To install from a named Git branch, pass `--branch`:

```bash
aiskills install owner/repo --branch develop
```

See [Named Branches](commands/install.md#named-branches) for how the branch is validated and tracked.
...
Search the skills.sh and agentskill.sh marketplaces, or your locally installed skills. The query must be at least 2 characters. After selecting results, you can `read`, `install` (marketplace only), `list`, or `finish`.
